<!DOCTYPE html>
<html>

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="Start your development with a Design System for Bootstrap 4.">
    <meta name="author" content="Creative Tim">
    <title>Art Coins Web</title>
    @include('links.headLinks')
</head>

<body>
    @include('footAndHead.headerUser')
    <main>
        <section class="section section-lg myBgBlue">
            <div class="container">
                <div class="row justify-content-center text-center mb-lg">
                    <div class="col-lg-8">
                        <h2 class="display-3">ES HORA DE GANAR UNAS MONEDITAS</h2>
                        <p class="lead text">Visualiza nuestro contenido y apresurate a ganar moneras!</p>
                        <p class="lead text">Recuerda que cada 3 horas aparecerá nuestro anuncio especial con el que podrás obtener Art Coins extra!!</p>
                    </div>
                </div>
                <div class="row">
       <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="30000" name="artcoins">                   
                            <input  type="image" alt="submit" src="data:image/png;base64," class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">50 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                   
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="10" name="artcoins">                   
                            <input  type="image" alt="submit" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQgHNO6eQHSZPaE-QLoNUOvJgfOOJVzoEyObnu7O-6WGggLSIo4" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">10 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                     <div class="col-md-6 col-lg-3 mb-5 mb-lg-0">
                        <div class="px-4">
                              <form role="form" action="{{action('PersonController@coinWined')}}" method="post">
                                   <input type="hidden" value="5" name="artcoins">                   
                            <input  type="image" alt="submit" src=" https://insidefmcg.com.au/wp-content/uploads/2017/10/McDonalds-Ad-Australia-2.jpg" class=" img-center img-fluid shadow shadow-lg--hover" style="width: 200px;" >
                            </form>
                            <div class="pt-4 text-center">
                                <h5 class="title">
                                    <span class="d-block mb-1">visualiza Video</span>
                                    <small class="h6 text-muted">5 Art Coins</small>
                                </h5>
                            </div>
                                  
                        </div>
                    </div>
                    
         


 
                  
                    

                </div>
            </div>
        </section>

    </main>
    @include('footAndHead.footer')
    @include('links.footLinks')
</body>

</html>
